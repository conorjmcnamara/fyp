...
class DblpParser(DatasetParser):
    ...
    def _parse_v10_paper(self, paper_json: Dict[str, Any]) -> Paper:
        paper = Paper()
        paper.id = paper_json.get("id")
        paper.title = paper_json.get("title")
        paper.authors = paper_json.get("authors")

        year = paper_json.get("year")
        if year and year >= self.config.train_years[0]:
            paper.year = year

        paper.abstract = paper_json.get("abstract")
        paper.venue = paper_json.get("venue")
        paper.references = paper_json.get("references")
        return paper

    def _parse_v12_paper(self, paper_json: Dict[str, Any]) -> Paper:
        paper = Paper()
        paper.id = paper_json.get("id")
        paper.title = paper_json.get("title")
        ...
        indexed_abstract = paper_json.get("indexed_abstract", {})
        ...
        paper.venue = paper_json.get("venue", {}).get("raw")
        paper.references = paper_json.get("references")
        return paper
